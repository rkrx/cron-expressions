...
const t = (expression: string, offsetDate: string, expectedResult: string) => {
    return () => {
        ...
        assertEquals(expectedResult, format(nextDate, 'yyyy-MM-dd HH:mm:ss', undefined));
    };
}
...
Deno.test('next day, where day is */5', t('0 0 */5 * *', '2000-02-01 00:00:00', '2000-02-06 00:00:00'));
//endregion
...
function parse(date: string): Date {
    const matches = date.match(/^(\d{4})\-(\d{2})\-(\d{2})\D(\d{2}):(\d{2}):(\d{2})$/);
    return new Date(
        matches ? parseInt(matches[1]) : 0,
        matches ? parseInt(matches[2]) - 1 : 0,
        matches ? parseInt(matches[3]) : 0,
        matches ? parseInt(matches[4]) : 0,
        matches ? parseInt(matches[5]) : 0,
        matches ? parseInt(matches[6]) : 0,
        0
    );
}
